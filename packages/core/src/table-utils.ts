...
import { createHtmlTableGrid, type HtmlTableCellRef } from './grid.js';
...
export function applyColumnWidths(table: ProseMirrorNode, widths: number[]): ProseMirrorNode {
  ...
  const insertIndex = tableChildren.findIndex((child) =>
...
function findTableContext(doc: ProseMirrorNode, selection: Selection): TableContext | undefined {
